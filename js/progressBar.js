...
class ProgressBar {
	...
	drawProgress() {
		...
		this.progressGraphics.drawRect(
			this.x,
			this.y,
			Math.min(this.value, this.maxWidth) * this.unit,
			this.height
		);
	...
	drawThreshold() {
		...
		this.thresholdGraphics.drawRect(
			this.x + this.threshold * this.unit,
			this.y - 5,
			5,
			this.height + 10
		);
...
class DoubleProgressbar {
	constructor(
		...
		this.backgroundGraphics = Game.add.graphics(0, 0);
		Game.world.bringToTop(this.backgroundGraphics);
		this.honeyProgress = new ProgressBar(
			'Honey:',
			tmpX,
			y,
			maxWidth,
