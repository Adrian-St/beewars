...
class Game {
	...
	spawnEnemy() {
		var wasp = new Wasp(this.lastWaspID, this);
	...
	getFlowerForPosition(position) {
		for (let i = 0; i < this.flowers.length; i++) {
			if (
				this.flowers[i].x === position.x &&
				this.flowers[i].y === position.y
			)
	...
	handleBuilding(workingBee) {
		if (this.beehive.honey >= 10) {
			...
			this.sendMessage('Not enough honey for building', workingBee.playerActions[0].playerIDs);
		}
		connection.updateBeehive(this.beehive, this.roomName);
	}
	...
	produceGeleeRoyal(workingBee) {
		if (this.beehive.pollen >= 5) {
			...
			this.sendMessage('Not enough pollen for producing gelee-royal', workingBee.playerActions[0].playerIDs);
		}
		connection.updateBeehive(this.beehive, this.roomName);
	}
	...
	handleCleaning(workingBee) {
		if (this.beehive.dirtyHoneycombs > 0) {
			...
			this.sendMessage('There are no honeycombs to be cleaned', workingBee.playerActions[0].playerIDs);
		}
		connection.updateBeehive(this.beehive, this.roomName);
	}
